...
export default function AdminRevenuePage() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as PaymentRecord));
      setPayments(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  if (authLoading || loading || !isAdmin) {
    return (
      <div className="flex min-h-screen bg-[#080b0f]">
        <Sidebar navItems={navItems} activeItem="revenue" onNavChange={() => {}} variant="admin" />
        <main className="flex-1 lg:ml-[240px] p-6 pt-20 lg:pt-6 flex items-center justify-center">
          <div className="animate-pulse text-cyber-muted">Loading revenue...</div>
        </main>
      </div>
    );
  }

  // Calculate MRR and total revenue (amounts are in kobo, divide by 100)
  const totalRevenue = payments.reduce((sum, p) => sum + (p.amount / 100), 0);
  
  // Basic MRR estimate: assume all payments in the last 30 days are recurring
      ...
      <main className="flex-1 lg:ml-[240px] p-6 pt-20 lg:pt-6 overflow-x-hidden">
        ...
        >
          <div>
            ...
            <p className="text-sm text-cyber-muted">Financial metrics and transaction history.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-cyber-card border border-cyber-border rounded-xl p-5">
              <p className="text-[10px] font-bold text-cyber-muted uppercase tracking-widest mb-2">Total Revenue</p>
              <p className="text-2xl font-bold text-cyber-green">₦{totalRevenue.toLocaleString()}</p>
            </div>
            <div className="bg-cyber-card border border-cyber-border rounded-xl p-5">
              <p className="text-[10px] font-bold text-cyber-muted uppercase tracking-widest mb-2">Est. MRR (30 Days)</p>
              <p className="text-2xl font-bold text-cyber-cyan">₦{mrr.toLocaleString()}</p>
            </div>
            <div className="bg-cyber-card border border-cyber-border rounded-xl p-5">
              <p className="text-[10px] font-bold text-cyber-muted uppercase tracking-widest mb-2">Total Transactions</p>
              <p className="text-2xl font-bold text-cyber-purple">{payments.length}</p>
            </div>
          </div>
          ...
          <div className="bg-cyber-card border border-cyber-border rounded-xl p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {payments.map((p) => (
                    <tr key={p.id} className="border-b border-cyber-border/30 hover:bg-cyber-border/10">
                      <td className="px-4 py-3 whitespace-nowrap text-cyber-muted">{new Date(p.createdAt).toLocaleDateString()} {new Date(p.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                      <td className="px-4 py-3 font-medium text-white">{p.email}</td>
                      <td className="px-4 py-3 text-cyber-green font-mono">₦{(p.amount / 100).toLocaleString()}</td>
                      <td className="px-4 py-3">
                        <span className="bg-cyber-green/10 text-cyber-green text-[10px] px-2 py-0.5 rounded-full border border-cyber-green/20 uppercase tracking-wide">
                          {p.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-mono text-[10px] text-cyber-muted">{p.reference}</td>
                    </tr>
                  ))}
                  {payments.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-cyber-muted">No transactions found</td>
                    </tr>
                  )}
                </tbody>
              </table>
